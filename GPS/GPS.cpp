...
GPS_status_enum
GPS::readNMEA(char *nmeaSentenceBuffer)
{
  ...
  while (1)
    {
      if (millis() - time > m_millisecondsTimeout)
        return GPS_TIMEOUT;
      ...
      if (state == 6)
        {
          ...
          if (offset < MAX_NMEA_SENTENCE_LENGTH - 3)
            {
  ...
  if (m_out != NULL)
    m_out->print(nmeaSentenceBuffer);
  return GPS_OK;
}

int
GPS::findStartOfFieldOffset(char *nmeaSentenceBuffer, int fieldNumber)
{
  if ((fieldNumber < 1))
    return -1;
  ...
  for (int i = 0; i < MAX_NMEA_SENTENCE_LENGTH; i++)
    {
      if (nmeaSentenceBuffer[i] == '\0')
        return -1;

      if (nmeaSentenceBuffer[i] == ',')
        fieldNumber -= 1;
